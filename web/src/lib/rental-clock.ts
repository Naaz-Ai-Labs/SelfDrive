/**
 * ⚠️ DUPLICATE OF `crm/src/lib/rental-clock.ts` — KEEP THE TWO BYTE-EQUIVALENT.
 *
 * The website and the CRM are separate deployments with separate node_modules and no
 * shared package, so this module is copied rather than imported. It is pure (no DB, no
 * clock reads, no config, no dependencies), which makes duplication safe — but it also
 * means an edit here that is not mirrored in the CRM copy silently makes the site quote
 * one price while the CRM charges another. That divergence is exactly the bug this file
 * was created to end. If you change anything below, change the CRM copy in the same
 * commit.
 *
...
export function computeRentalDays(input: RentalClockInput): RentalClockResult {
  ...
  const isSaturdayPickup = istParts(pickupAt).weekday === 6;
  const isSaturdayReturn = istParts(returnAt).weekday === 6;
  const spanDays = calendarDaysBetween(pickupAt, returnAt);
  const rawDays = Math.max(1, spanDays + (lateDrop ? 1 : 0));
  // A rental starting on Saturday or dropping off on Saturday locks the vehicle for the full weekend package:
  // - Pickup on Saturday: Saturday + Sunday (2 days minimum)
  // - Drop-off on Saturday (from Friday or any other weekday): must include through Sunday (spanDays + 2 days)
  let days = rawDays;
  if (isSaturdayPickup) {
    days = Math.max(2, days);
  }
  if (isSaturdayReturn) {
    days = Math.max(spanDays + 2, days);
  }
