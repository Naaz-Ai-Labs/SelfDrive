...
async function checkBlocks() {
  ...
  const { data: blocks, error: bErr } = await supabase.from("availability_blocks").select("*");
  console.log("=== availability_blocks ===", blocks, bErr);

  const { data: branches, error: brErr } = await supabase.from("branches").select("*");
  console.log("=== branches ===", branches, brErr);
}
