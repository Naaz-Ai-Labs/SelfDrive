  ...
  const body = (
    <>
      <div className="flex items-start justify-between gap-2">
        ...
        {icon && (
          <span className={cn("flex h-8 w-8 shrink-0 items-center justify-center rounded-full", badgeMap[accent ?? "ink"])} aria-hidden>
            {icon === "₹" ? (
              <span className="text-base font-bold leading-none">₹</span>
            ) : (
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d={icon} /></svg>
            )}
          </span>
        )}
      </div>
      <div className="mt-2 flex items-baseline gap-2">
        <p className={cn("font-display text-2xl font-semibold", accentMap[accent ?? "ink"])}>{value}</p>
        {trend && (
          <span className={cn("flex items-center gap-0.5 text-xs font-semibold", trend.positive ? "text-emerald-600" : "text-red-600")}>
            <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round" className={trend.positive ? "" : "rotate-180"}><path d="M12 19V5M5 12l7-7 7 7" /></svg>
            {Math.abs(trend.pct)}%
          </span>
        )}
      </div>
      ...
      {Array.from({ length: pages }, (_, i) => i + 1).map((p) => (
        <Link
          ...
          className={cn(
            ...
            p === page ? "bg-brand-600 text-white" : "border border-ink-200 bg-white text-ink-600 hover:border-brand-400"
          )}
        >
          {p}
        </Link>
